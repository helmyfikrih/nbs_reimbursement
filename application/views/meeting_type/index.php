    ...
    <div class="main-content-inner">
        <div class="breadcrumbs ace-save-state" id="breadcrumbs">
            <ul class="breadcrumb">
                ...
                <li><a href="#"><?= $this->lang->line('text_settings') ?></a></li>
                <li><a href="#"><?= $this->lang->line('text_master_data') ?></a></li>
                <li class="active"><?= $this->lang->line('meettype_bread') ?></li>
            </ul><!-- /.breadcrumb -->
        </div>

        <div class="page-content">
            <div class="page-header">
                <h1><?= $this->lang->line('meettype_head') ?></h1>
            </div><!-- /.page-header -->
            <div class="pull-right">
                ...
                <div class="btn-group">
                    <a id="add-group" href="javascript:;" onclick="clearData()" class=" btn btn-white btn-info btn-bold">
                        ...
                        <?= $this->lang->line('meettype_create') ?>
                    </a>
                </div>
            </div>
            <div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-6 col-sm-6">
                            <table border="0" width="100%">
                                ...
                                <tr>
                                    <td><?= $this->lang->line('meettype_name') ?></td>
                                    <td><input type="text" placeholder="<?= $this->lang->line('meettype_name') ?>" id="meetingTypeName" name="meetingTypeName" /></td>
                                </tr>
                                <tr>
                                    <td></td>
                                    <td>
                                        <button name="" id="" class="btn blue" onclick="input_meeting_type_data()">Submit</button>&nbsp;
            ...
            <div class="dataTables_borderWrap">
                <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                    <thead>
                        <tr>
                            <th><?= $this->lang->line('meettype_text') ?> ID</th>
                            <th><?= $this->lang->line('meettype_create') ?></th>
                            <th></th>
                        </tr>
                    </thead>
    ...
    jQuery(function($) {
        ...
        var myTable =
            ...
            .DataTable({
                ...
                "bServerSide": true,
                "ajax": {
                    url: "<?= base_url("meeting_type/getList/"); ?>",
                    type: "POST",
                    data: function(d) {
...
*/
    ...
    function input_meeting_type_data() {
        if (jQuery('#meetingTypeName').val() === '') {
            Toast.fire({
                icon: 'warning',
        ...
        ngeklik();
        // loading("tab-content");
        var meetingTypeId = jQuery("#meetingTypeId").val();
        var meetingTypeName = jQuery("#meetingTypeName").val();
        jQuery.ajax({
            url: "<?= base_url() ?>meeting_type/save",
            dataType: "json",
            type: "POST",
    ...
    function clearData() {
        jQuery("#meetingTypeId").val("");
        jQuery("#meetingTypeName").val("");
    ...
    function editData(e) {
        ...
        jQuery.ajax({
            ...
            url: "<?= base_url() ?>meeting_type/getOne",
            dataType: "json",
            success: function(e) {
                jQuery("#meetingTypeId").val(e[0].meetType_id);
                jQuery("#meetingTypeName").val(e[0].meetType_name);
            }
        });
...
<script>
    function deleteData(id) {
        ...
        var text = '<?= $this->lang->line('meettype_warning_delete') ?>'
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: '<?= base_url('meeting_type/deleteData/') ?>' + id,
                    type: 'POST',
                    data: {},
                    dataType: 'json',
