...
class Forum_model extends CI_model
{
    ...
    function getDataList($rowno, $rowperpage){
        ...
        $this->db->join('kms_user ku','ku.user_id=kf.user_id','left');
        $this->db->join('kms_schools ksc','ku.school_id=ksc.school_id','left');
        $this->db->join('kms_user_detail kud','ku.user_id=kud.user_id','left');
    ...
    public function getSingleForum($forum_id,$forum_slug){
        $query = "  SELECT 
                    ...
                    FROM 
                        kms_forum kf
                    LEFT JOIN kms_user ku ON ku.user_id=kf.user_id
                    LEFT JOIN kms_schools ksc ON ku.school_id=ksc.school_id
                    LEFT JOIN kms_user_detail kud On kud.user_id=ku.user_id
                    LEFT JOIN kms_user_role kur On kur.role_id=ku.role_id
                    WHERE 
    ...
    function getDataCommentList($rowno, $rowperpage, $forumId)
    {
        ...
        $this->db->join('kms_user ku', 'ku.user_id=kfc.user_id', 'left');
