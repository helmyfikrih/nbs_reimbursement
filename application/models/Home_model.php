...
class Home_model extends CI_model
{
    ...
    function getForum(){
        ...
        $this->db->join('kms_user_detail kud','kud.user_id=kf.user_id','left');
    ...
    function getUsers(){
        $this->db->select('ku.*, kud.ud_img_name, kur.role_name, ksc.school_name');
    ...
    function getSearchList($rowno, $rowperpage, $q)
    {
        $query = "SELECT 
                    ...
                    FROM 
                        kms_forum kf 
                    LEFT JOIN 
                        kms_user ku ON ku.user_id=kf.user_id
                    WHERE 
                        (forum_title LIKE '%$q%') 
                        AND forum_status=1
                    ORDER BY title ASC
