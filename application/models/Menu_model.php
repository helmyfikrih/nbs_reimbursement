...
Class Menu_model extends CI_Model
{
	function getmenu($user_id, $username)
	{
		...
		$sqlmenu0 		= "select role_id from kms_user where user_id = ".$user_id." and user_username = '".$username."'";
		...
		// die($role_id);
		$sqlmenu 		= "select role_allow_menu from kms_user_role where role_id = $role_id";
		...
		// $menu_no 	= str_replace(',','","',$allow_menu);
		$body		= '';
		$sqlmenu 	= 'select menu_id, menu_url, menu_name, menu_nama, menu_parent, menu_child, menu_icon from kms_menu where menu_status = 1 and menu_parent = 0 '. $in_menu.' order by menu_sort asc';
		// die($sqlmenu);
		...
		foreach ($result->result_array() as $line) {
			...
			if ( $line['menu_child'] == 0 ) {
				...
				$sqlGetParentFromUri2 = "select * from kms_menu where menu_url like '%".$uri."%'";
                $prentId2 = $this->db->query($sqlGetParentFromUri2)->row();
				$sqlGetParentFromUri = "select * from kms_menu where menu_id = ".$prentId2->menu_parent;
				...
				if(isset($prentId))
				{
                    ...
                    $sqlMenuParentActive = "SELECT * FROM kms_menu WHERE menu_url = '$uri' AND menu_status=1";
                    $parentActive = $this->db->query($sqlMenuParentActive)->result_array() or die('died 66');
                    $idParentActive = $parentActive[0]['menu_parent'];

                    $sqlMenuParentActive2 = "SELECT * FROM kms_menu WHERE menu_id = '$idParentActive' AND menu_status=1";
                ...
                $classActive = '';
                $isparentActive = '';
				$sqlmenu2 	= 'select * from kms_menu where menu_status = 1 and menu_parent = '.$line['menu_id'].' '. $in_menu.' order by menu_sort asc';
				...
				foreach ($result2->result_array() as $line2) {
					...
					if ( $line2['menu_child'] == 0 ) {
						...
						$sqlmenu3 	= 'select * from kms_menu where menu_status = 1 and  menu_parent = '.$line2['menu_id'].' '. $in_menu.' order by menu_sort asc';
						// die($sqlmenu3);
						...
						foreach ($result3->result_array() as $line3) {
							...
							if ( $line3['menu_child'] == 0 ) {
								...
								$classActive = '';
									...
									$sqlmenu4 	= 'select * from kms_menu where status = 1 and  menu_parent = '.$line3['menu_id'].' '.$in_menu.' order by menu_sort asc';
	...
	function UserAllow($role_id){
		$query ="SELECT * FROM kms_user_role WHERE role_id = $role_id LIMIT 1";
        $q = $this->db->query($query);
        return $q->result_array();
	}
	
	function idMenu($urlmenu){
		$query ="SELECT menu_id, menu_access FROM kms_menu WHERE menu_status = 1 and menu_url = '$urlmenu' ORDER BY menu_id ASC LIMIT 1";
        $q = $this->db->query($query);
        return $q->result_array();
	}
	
	function routing($id){
		$query ="SELECT menu_id,menu_url,menu_name,menu_access FROM kms_menu WHERE menu_status = 1 and menu_parent = $id AND menu_id<>$id  ORDER BY menu_sort";
        $q = $this->db->query($query);
        return $q->result();
	}
	...
	function cektreeup($id)
	{
		...
		$data = $this->db->get('kms_menu')->result_array();
